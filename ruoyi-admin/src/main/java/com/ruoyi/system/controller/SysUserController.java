...
import com.ruoyi.common.domain.Ztree;
...
public class SysUserController extends BaseController {
    ...
    public R addSave(@Validated SysUser user) {
        ...
        if (!userService.checkLoginNameUnique(user)) {
            return R.fail("新增用户'" + user.getLoginName() + "'失败，登录账号已存在");
        } else if (StringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user)) {
            return R.fail("新增用户'" + user.getLoginName() + "'失败，手机号码已存在");
        } else if (StringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user)) {
            return R.fail("新增用户'" + user.getLoginName() + "'失败，邮箱账号已存在");
        }
    ...
    public R editSave(@Validated SysUser user) {
        ...
        roleService.checkRoleDataScope(user.getRoleIds());
        if (!userService.checkLoginNameUnique(user)) {
            return R.fail("修改用户'" + user.getLoginName() + "'失败，登录账号已存在");
        } else if (StringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user)) {
            return R.fail("修改用户'" + user.getLoginName() + "'失败，手机号码已存在");
        } else if (StringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user)) {
            return R.fail("修改用户'" + user.getLoginName() + "'失败，邮箱账号已存在");
        }
    ...
    public R changeStatus(SysUser user) {
        userService.checkUserAllowed(user);
        userService.checkUserDataScope(user.getUserId());
        return R.ok(userService.changeStatus(user));
    }
